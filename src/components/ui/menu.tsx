...
const MobileNav = ({ menuExp, setMenuExpanded, closeMenu }: MenuExp & { closeMenu: () => void }) => {
	...
	return (
		<div className="absolute flex flex-col w-3/4 shadow h-auto justify-center z-10">
			{/* Menu */}
			<div className={`${menuExp ? "flex" : "hidden"} shadow flex-col p-6 absolute top-1 px-8 w-3/4 ml-11 rounded-xl transition duration-500 bg-white`}>
	...
	return (
		<div className={`${menuExpanded ? "mb-44 pb-16" : "pb-7"} `}>
			<div className="flex w-auto md:invisible justify-center hover:cursor-pointer p-1">
				<Icon className="transition duration-100" size={25} icon={menuExpanded ? CloseIcon : MenuIcon} onClick={() => setMenuExpanded(!menuExpanded)} />
			</div>
			{menuExpanded && (
