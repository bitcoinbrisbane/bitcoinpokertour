...
router.post("/:eventid", async (req, res) => {
	const event_id = req.params.eventid;
	const { name, email, bitcoin_address, pgp_key } = req.body;
	...
	if (pgp_key) {
		const result = await axios.get(pgp_key);
	...
	let textBody = "Thank you for registering for the event. Your bitcoin address is: " + registration.bitcoin_address;

	if (pgp_key) {
		const passphrase = process.env.PGP_KEY_PASSWORD || `yourPassphrase`;
		...
		const encrypted = await openpgp.encrypt({
			message: await openpgp.createMessage({ text: textBody }),
			encryptionKeys: pgp_key,
